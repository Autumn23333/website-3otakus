        ...
        if(loginUser == null){
            ...
            if(null!=cookies){
                for(Cookie cookie : cookies){
                    ...
                    }else if("upid".equals(cookie.getName())){
                        loginCookiePassword = LoginTool.cookiep2pwd(cookie.getValue());
                    }
                    //}
                }
                if(!"".equals(loginCookieUserName) && !"".equals(loginCookiePassword)){
                    User user = userServ.checkUserDirectLogin(loginCookieUserName, loginCookiePassword);
                    if(user != null){
                        request.getSession().setAttribute("loginUser", user);
                    } else {
                        //删除登录cookie
                        Cookie userNameCookie = new Cookie("uname", "");
                        Cookie passwordCookie = new Cookie("upid", "");
